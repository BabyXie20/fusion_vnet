...
class WaveletDown(nn.Module):
    ...
    def __init__(
        ...
        # body 负责提特征 + 作为 skip
        self.body = convBlock(2, in_ch, out_ch, normalization=normalization)
        self.dwt  = DWT3D()
...
class BottleneckCluster3D(nn.Module):
    ...
    瓶颈结构（三分支）：
        ...
        并联三支：
            1) local_branch   : MultiScaleLocal3D，本地多尺度卷积分支
            2) cluster_branch : Cluster3DVolume，聚类稀疏注意力（全局）
            3) stripe_branch  : StripeConvAttention3D，条状卷积 + 条状注意力（细长器官）
    ...
    def __init__(
        ...
        self.conv_in = nn.Sequential(
            FDWConv3d(
                ...
                stride=1,
                padding=1,
                kernel_num=4,      # 4 个频带核，可以先试 3 or 4
                reduction=8        # 通道注意力瓶颈
            ),
        ...
        # 2) 本地分支：MultiScaleLocal3D（保持空间尺寸不变，通道不变）
        self.local_branch = MultiScaleLocal3D(c5, normalization=normalization)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x       : [B,C,D,H,W]    —— 这里是瓶颈低频特征 l5
        dwt_high: [B,C,7,D,H,W] 或 None —— 对应第 4 层的高频子带 h5
        """
        B, C, D, H, W = x.shape
